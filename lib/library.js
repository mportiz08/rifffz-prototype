(function() {
  var EventEmitter, Library, redis, util, _,
  ...
  Library = (function(_super) {
    ...
    Library.prototype.addAlbum = function(json) {
      var resource, song, _i, _len, _ref, _results;
      this.addArtist(json.artist.name);
      resource = "artist:" + (util.slugify(json.artist.name)) + ":album:" + (util.slugify(json.album.name));
      this.setVal(resource, json.album.name);
      this.setVal("" + resource + ":year", json.album.year);
      this.setVal("" + resource + ":cover", json.album.cover);
      _ref = json.album.songs;
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        song = _ref[_i];
        _results.push(this.client.rpush("" + resource + ":songs", song));
      }
      return _results;
    };
